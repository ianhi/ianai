...
def run_tests():
    ...
    result = fs.move(
        "test_fs_operations/renamed_file.txt",
        "test_fs_operations/subfolder/renamed_file.txt",
    )
    ...
    result = fs.copy(
        "test_fs_operations/test_file.txt",
        "test_fs_operations/test_file.txt",
        overwrite=False,
    )
    ...
    print("\n\n### TEST 16: Create Nested Directories ###")
    result = fs.create_directory(
        "test_fs_operations/level1/level2/level3", parents=True
    )
    ...
    try:
        ...
        print_result(
            "Try to delete system file (should be blocked)",
            {
                "success": True,
